...
module.exports = function(app) {
  ...
  app.set('JWT_SECRET', process.env.JWT_SECRET || 'SomeSecret')
  app.set('JWT_COOKIE_NAME', process.env.JWT_COOKIE_NAME || 'Andrei Token')
...
/**
   *  Andrei's Comments on Configuration Variables
   ...
   *  PORT , PG_HOST, PG_USER, PG_PASSWORD, PG_DB, JWT_SECRET
      Also, the following non-security related information should also be set for use elsewhere:
      CORS_CONFIG

       We can use the app.set and process.env to retrieve environment variables, and provide a fallback
   *  if any are not defined. For example: app.set('PG_HOST', process.env.PG_HOST || 'localhost')
   */
